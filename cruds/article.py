...
def getArticleList():
    ...
    for article in query.select().where(Article.is_publish==TRUE).order_by(Article.good_count.desc()).limit(10):
        ...
        articles.append(
            {"relate_user_id": article.relate_user_id, "relate_user_name": relate_user_name, "tags": tags, "title": article.title, "good_count": article.good_count,  "post_date": article.post_date})
